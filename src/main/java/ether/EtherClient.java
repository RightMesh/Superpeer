...
public final class EtherClient {
    ...
    public static String getEtherBalance(String address, Http httpAgent) throws IOException{
        ...
        String queryEtherBalanceString="{\"method\":\"eth_getBalance\",\"params\":[\""+ address+"\"],\"id\":42,\"jsonrpc\":\"2.0\"}";
    ...
    public static String getTokenBalance(String address, Http httpAgent) throws IOException{
        ...
        if(Settings.DEBUG_INFO) {
            System.out.println("Balance of " + address + " = " + new Float(new BigInteger(tokenBalance.substring(2), 16).doubleValue() / (new BigInteger(Settings.APPENDING_ZEROS_FOR_TOKEN, 10).doubleValue())).toString() + " TKN");
        }
     ...
     * @throws IOException
     */
    public static EtherUtility.PaymentChannel getChannelInfo(String senderAddress, String receiverAddress, Http httpAgent) throws DecoderException, IOException {

        byte[] keyInBytes= EtherUtility.getChannelHash(senderAddress, receiverAddress);

        CallTransaction.Function channels = Settings.CHANNEL_CONTRACT.getByName("channels");
        byte [] functionBytes=channels.encode(keyInBytes);
        String requestString = "{\"method\":\"eth_call\"," +
                "\"params\":[" +
                "{" +
                "\"to\":\""+Settings.CHANNEL_ADDRESS+"\"," +
                "\"data\":\""+"0x" + new String(org.apache.commons.codec.binary.Hex.encodeHex(functionBytes))+"\"" +
                "}," +
                "\"latest\"" +
                "]," +
        ...
        return new EtherUtility.PaymentChannel("0x111", "0x222", Settings.MAX_DEPOSIT, 1, null, null);
    }
     ...
     * @throws IOException
     */
    public boolean checkChannelAvailable(String senderAddress, String receiverAddress, Http httpAgent) throws DecoderException, IOException{

        byte[] senderAddressBytes;
        try{
            senderAddressBytes = EtherUtility.etherAddressToByteArray(senderAddress);
        }catch (DecoderException e){
            if(Settings.DEBUG_INFO) {
                System.out.println("Invalid sender address: "+senderAddress);
            }
            throw e;
        }
        ...
        try{
            recvAddressBytes = EtherUtility.etherAddressToByteArray(receiverAddress);
        }catch (DecoderException e){
            if(Settings.DEBUG_INFO) {
                System.out.println("Invalid receiver address: "+receiverAddress);
            }
            throw e;
        }


        byte[] data = EtherUtility.concatenateByteArrays(senderAddressBytes, recvAddressBytes);

        if(Settings.DEBUG_INFO) {
            System.out.println("The keccak256 argument of bytes in string "+Hex.encodeHexString(data));
        }

        byte[] keyInBytes= Digests.keccak256().reset().update(data).digest();
        CallTransaction.Function channels = Settings.CHANNEL_CONTRACT.getByName("channels");
        byte [] functionBytes=channels.encode(keyInBytes);
        String requestString = "{\"method\":\"eth_call\"," +
                "\"params\":[" +
        ...
        if(Settings.DEBUG_INFO) {
            System.out.println("There " + (new BigInteger(checkChannelAvailableResult.substring(64), 16).compareTo(BigInteger.ZERO) > 0 ? "is" : "isn't") + " a channel from " + senderAddress + " to " + receiverAddress);
        }
     ...
     */
    public static boolean approve(String senderAddress, BigInteger deposit, String signedApproveTrans, Http httpAgent) throws NumberFormatException, IOException, IllegalArgumentException {

        if(Settings.DEBUG_INFO) {
            System.out.println(senderAddress + " tries to approve channel " + Settings.CHANNEL_ADDRESS + " up to " + deposit + " Tokens at maximum.");
        }

        //TODO: check this
        CallTransaction.Function approve= Settings.TOKEN_CONTRACT.getByName("approve");
        byte [] approveFunctionBytes=approve.encode(Settings.CHANNEL_ADDRESS, deposit);
        ...
        if(approveGasEstimate.compareTo(Settings.GAS_LIMIT) > 0){
            if(Settings.DEBUG_INFO) {
                System.out.println("Estimeted GAS for approve transaction is too high. Probably some arguments are Invalid.");
            }
            throw new IllegalArgumentException("Estimeted GAS for approve transaction is too high. Probably some arguments are Invalid.");
        }
     ...
     * @throws IllegalArgumentException
     */
    public static EtherUtility.PaymentChannel openChannel(String senderAddress, String receiverAddress, BigInteger deposit, String signedOpenChannelTrans, Http httpAgent)
            throws IOException, IllegalArgumentException {

        if(Settings.DEBUG_INFO) {
            System.out.println(senderAddress + " tries to open a channel to pay " + receiverAddress + " up to " + deposit + " Tokens at maximum.");
        }
        ...
        if(estimatedGas.compareTo(Settings.GAS_LIMIT) > 0)
            throw new IllegalArgumentException("Exceeded GAS estimation. Probably invalid signed transaction provided.");
        ...
        if(Settings.DEBUG_INFO){
            System.out.println(senderAddress+" --> "+receiverAddress + " channel has been opened in block "+new BigInteger(blockNumberHex.substring(2),16).toString(10));
        }
    ...
    public static void cooperativeCloseSender(MeshID ownMeshId, String senderAddress, String receiverAddress, byte[] balance_Msg_Hash_Sig_Sender, byte[] closing_Msg_Hash_Sig_Receiver, String balance, String signedCooperativeCloseSenderTransParam, Http httpAgent) throws NumberFormatException, IllegalArgumentException, IOException{
        ...
        byte [] cooperativeCloseSenderFunctionBytes=cooperativeCloseSender.encode(receiverAddress,
                tempBalance,balance_Msg_Hash_Sig_r,balance_Msg_Hash_Sig_s,new BigInteger(balance_Msg_Hash_Sig_v),closing_Msg_Hash_Sig_r,closing_Msg_Hash_Sig_s,new BigInteger(closing_Msg_Hash_Sig_v));
        String querycooperativeCloseGasString = "{\"method\":\"eth_estimateGas\"," +
                ...
                "\"to\":\""+Settings.CHANNEL_ADDRESS+"\"," +
                "\"value\":\""+"0x"+new BigInteger("0",10).toString(16)+"\","+
                "\"data\":\""+"0x" + new String(org.apache.commons.codec.binary.Hex.encodeHex(cooperativeCloseSenderFunctionBytes))+"\"" +
                "}" +
                "]," +
                "\"id\":42,\"jsonrpc\":\"2.0\"}";
        ...
        if(signedCooperativeCloseSenderTrans == null || signedCooperativeCloseSenderTrans.isEmpty()) {
            ...
            Transaction cooperativeCloseSenderTrans = new Transaction(EtherUtility.bigIntegerToBytes(senderNonce), // nonce
                    ...
                    ByteUtil.hexStringToBytes(Settings.CHANNEL_ADDRESS), // to id
                    EtherUtility.bigIntegerToBytes(new BigInteger("0", 10)), // value
                    cooperativeCloseSenderFunctionBytes, 42);// chainid

            ownMeshId.sign(cooperativeCloseSenderTrans);
            signedCooperativeCloseSenderTrans = "0x" + new String(org.apache.commons.codec.binary.Hex.encodeHex(cooperativeCloseSenderTrans.getEncoded()));
        }
    ...
    public static void cooperativeCloseReceiver(MeshID ownMeshId, String senderAddress, String receiverAddress, byte[] balance_Msg_Hash_Sig_Sender, byte[] closing_Msg_Hash_Sig_Receiver, String balance, String signedCooperativeCloseReceiverTransParam, Http httpAgent) throws NumberFormatException, IllegalArgumentException, IOException{
        ...
        String querycooperativeCloseGasString = "{\"method\":\"eth_estimateGas\"," +
                ...
                "\"to\":\""+Settings.CHANNEL_ADDRESS+"\"," +
                "\"value\":\""+"0x"+new BigInteger("0",10).toString(16)+"\","+
                "\"data\":\""+"0x" + new String(org.apache.commons.codec.binary.Hex.encodeHex(cooperativeCloseReceiverFunctionBytes))+"\"" +
                "}" +
                "]," +
                "\"id\":42,\"jsonrpc\":\"2.0\"}";
        ...
        if(signedCooperativeCloseReceiverTrans == null || signedCooperativeCloseReceiverTrans.isEmpty()) {
            ...
            signedCooperativeCloseReceiverTrans = "0x" + new String(org.apache.commons.codec.binary.Hex.encodeHex(cooperativeCloseReceiverTrans.getEncoded()));
        }
    ...
    private static BigInteger parseBalance(String balance) throws NumberFormatException, IllegalArgumentException{
        ...
        if(Settings.MAX_DEPOSIT.compareTo(initDeposit)<0) {
            if(Settings.DEBUG_INFO) {
                System.out.println("Provided deposit is larger than maximum allowed " + Settings.MAX_DEPOSIT.toString(10));
            }
            throw new IllegalArgumentException("Provided deposit is larger than maximum allowed " + Settings.MAX_DEPOSIT.toString(10));
        }
